  ...
  try {
    ...
    if (res.code && TCB_ERR_CODE[res.code] !== 0) {
      ...
      const fileList = (res.fileList || []).filter(s => Boolean(s)).map((f, i) => {
        if (f.code && TCB_ERR_CODE[f.code] !== 0) {
          const code = TCB_ERR_CODE[f.code] || TCB_ERR_CODE.SYS_ERR;
          return {
            fileID: f.fileID,
            status: TCB_ERR_CODE[f.code] || TCB_ERR_CODE.SYS_ERR,
...
class cloud_Cloud {
  ...
  getTcbInstance({
    ...
    const newConfig = _objectSpread({}, this.config, {}, config);
  ...
  translateEnv(config = {}, type = 'default') {
    ...
    if (typeof newConfig.env === 'object') {
